...
import React__default, { memo, useState, useMemo as useMemo$1, useEffect, useRef, cloneElement, version, isValidElement, useImperativeHandle, forwardRef, createElement, useCallback } from 'react';
...
const Textarea = memo(forwardRef(({ className = '', disabled, error, startIcon, endIcon, ...props }, ref) => {
...
function Ge(t, e, n) {
  ...
  const s = React__default.useMemo(() => typeof ResizeObserver < "u" ? new ResizeObserver((i) => {
    const l = () => {
      const c = i[0].target;
...
const er = /* @__PURE__ */ U(() => {
  ...
  ([t, e]) => ({ ...t, ...e }),
  X(Yn, er)
), or = ({ height: t }) => /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { height: t } }), rr = { overflowAnchor: "none", position: Ye(), zIndex: 1 }, Qn = { overflowAnchor: "none" }, sr = { ...Qn, display: "inline-block", height: "100%" }, Cn = /* @__PURE__ */ React__default.memo(function({ showTopList: e = false }) {
...
const MessageComponent = memo(({ item, className = '', textMessageOverride, imageMessageOverride, videoMessageOverride, textMessageProps, imageMessageProps, videoMessageProps, timeLabelProps, avatarProps, responseLoadingComponent, }) => {
...
const useMessageCache = (conversationId, initial, cache) => {
    ...
    const saveToCache = useCallback((msgs) => {
        if (!cache)
            return;
...
const MessageListComponent = forwardRef(({ messages = [], conversationId = '', className = '', onLoadMorePreviousData, cache = false, showScrollToEndButton = true, emptyDataComponent, isPrevLoading = false, prevLoadingComponent, customMessageComponentProps, virtuosoProps, scrollToEndButtonProps, responseLoadingComponent, }, ref) => {
    ...
    const [hasShowScrollToEndButton, setHasShowScrollToEndButton] = useState(false);
